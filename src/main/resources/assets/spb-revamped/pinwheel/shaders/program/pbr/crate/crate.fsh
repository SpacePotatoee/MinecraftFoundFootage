...
void main() {
//    vec3 normal2 = viewToWorldSpaceDirection(normal);
//    vec3 randDir = normalize(vec3(1,0,0));
//    vec3 tangent = normalize(cross(normal, randDir));
...
//    TBN = transpose(TBN);
    ...
    for(int i = 1; i <= MaxSteps; i++){
        ...
        float heightMapDepth = ((1 - texture(HeightMap, vec2(texCoords.x, -texCoords.y) * ZOOM).r) * 0.15);
    ...
    fragAlbedo = vec4(color.rgb * 1 - (1 - aoMap.rgb), 1.0);
